...
const ContributorsPage: React.FC<ContributorsPageProps> = () => {
  ...
  const renderRankBadge = (rank: number) => {
    if (rank === 1) {
      return (
        <div className="w-8 h-8 rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-500 flex items-center justify-center font-bold text-xs shadow-sm ring-1 ring-yellow-500/20">
          <Trophy size={14} />
        </div>
      );
    }
    if (rank === 2) {
      return (
        <div className="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 flex items-center justify-center font-bold text-xs shadow-sm ring-1 ring-gray-400/20">
          <Medal size={14} />
        </div>
      );
    }
    if (rank === 3) {
      return (
        <div className="w-8 h-8 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-500 flex items-center justify-center font-bold text-xs shadow-sm ring-1 ring-orange-500/20">
          <Award size={14} />
        </div>
      );
    }
    return (
      <div className="w-8 h-8 rounded-full bg-gray-50 dark:bg-[#111] text-gray-400 dark:text-gray-500 border border-gray-200 dark:border-[#333] flex items-center justify-center font-bold text-xs">
        #{rank}
      </div>
    );
  };
    ...
    <div className="flex flex-col h-full min-h-[500px]">
      <div className="flex gap-3 sm:flex-row sm:items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-black dark:text-white">Contributors</h2>
            <p className="text-xs text-gray-500 font-medium">{total} total members helping the community</p>
          </div>
        </div>
      ...
      {!isLoading && profile?.username && (
        <div className="mb-8 p-[1px] rounded-2xl bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-blue-500/20 dark:from-blue-500/10 dark:via-purple-500/10 dark:to-blue-500/10 shadow-sm animate-fade-in">
          <div className="bg-white/80 dark:bg-black/80 backdrop-blur-xl rounded-[15px] p-5 border border-white/20 dark:border-white/5 relative overflow-hidden">
            {/* Background elements */}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center relative z-10">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-gradient-to-br from-blue-50 dark:from-[#111] to-blue-100 dark:to-[#222] border border-blue-200 dark:border-[#333] rounded-full flex items-center justify-center shadow-sm text-xl font-bold text-blue-600 dark:text-white">
                  {profile.username.charAt(0)}
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base font-bold text-black dark:text-white">{profile.username}</h3>
                    <span className="px-2 py-0.5 rounded-full bg-black/5 dark:bg-white/10 text-[10px] font-semibold text-gray-500 dark:text-gray-400 flex items-center gap-1">
                      <Sparkles size={10} />
                      You
                    </span>
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400 font-medium mt-0.5">Keep adding valid links to climb the ranks!</p>
                </div>
              </div>
              
              <div className="flex items-center divide-x divide-gray-200 dark:divide-[#333] border border-gray-200 dark:border-[#333] rounded-lg bg-white/50 dark:bg-[#111]/50 self-stretch sm:self-auto">
                <div className="px-4 py-2 flex flex-col items-center">
                  <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wider mb-1">Your Rank</span>
                  <div className="flex items-center gap-1.5">
                    <Hash size={14} className="text-blue-500" />
                    <span className="text-sm font-bold text-black dark:text-white">{profile.rank || '-'}</span>
                  </div>
                </div>
                <div className="px-4 py-2 flex flex-col items-center">
                  <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wider mb-1">Links Added</span>
                  <div className="flex items-center gap-1.5">
                    <Activity size={14} className="text-green-500" />
                    <span className="text-sm font-bold text-black dark:text-white">{profile.links_added?.toLocaleString() || 0}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className="border-b border-gray-200 dark:border-[#333] bg-gray-50/50 dark:bg-[#111]/50 top-0 sticky z-10 backdrop-blur-sm">
                  <th className="font-semibold text-xs text-gray-400 uppercase tracking-wider py-3 px-4 w-16 text-center">Rank</th>
                  <th className="font-semibold text-xs text-gray-400 uppercase tracking-wider py-3 px-4">Contributor</th>
                  <th className="font-semibold text-xs text-gray-400 uppercase tracking-wider py-3 px-4 w-[25%] hidden sm:table-cell">Joined</th>
                  <th className="font-semibold text-xs text-gray-400 uppercase tracking-wider py-3 px-4 w-32 text-right">Links Added</th>
                  <th className="font-semibold text-xs text-gray-400 uppercase tracking-wider py-3 px-4 w-[20%]"></th>
                </tr>
              </thead>
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2.5">
                          <div className={`w-8 h-8 rounded flex items-center justify-center font-bold text-sm shrink-0 border ${
                            isMe 
                          ...
                          <div>
                            <div className="text-sm font-semibold text-black dark:text-white flex items-center gap-2">
                              ...
                              {isMe && <span className="text-[9px] bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded font-bold">YOU</span>}
                            </div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4 pr-6 w-32">
                        <div className="h-1.5 bg-gray-100 dark:bg-[#222] rounded-full overflow-hidden w-full flex items-center group-hover:bg-gray-200 dark:group-hover:bg-[#333] transition-colors">
                          <div 
